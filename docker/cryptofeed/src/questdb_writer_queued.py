# code is unused for now, but was created for future reference
...
#             fields = f"price={price},size={amount}"
...
#
#             # Build bid and ask arrays in QuestDB format: first row prices, second row volumes
#             # Convert Decimal to float for QuestDB double array serialization
#             bid_prices = []
#             bid_volumes = []
#             for i in range(min(bid_depth, max_bids)):
#                 price, size = book.book.bids.index(i)
#                 bid_prices.append(float(price))
#                 bid_volumes.append(float(size))
#
#             ask_prices = []
#             ask_volumes = []
...
#                 ask_volumes.append(float(size))
#
#             # QuestDB format: [[price1, price2, ...], [volume1, volume2, ...]]
#             bids = [bid_prices, bid_volumes]
#             asks = [ask_prices, ask_volumes]
#
#             # For QuestDB ILP, we need to format double arrays differently
#             # Instead of JSON, send as QuestDB native array format:
#             # [[price1,price2,...],[volume1,volume2,...]]
#             bids_str = f"[[{\ ','.join(map(str, bid_prices))}],[{\ ','.join(map(str, bid_volumes))}]]"
#             asks_str = f"[[{\ ','.join(map(str, ask_prices))}],[{\ ','.join(map(str, ask_volumes))}]]"
#
#             # Build ILP line: table,tags fields timestamp
#             tags = f"exchange={exchange},symbol={symbol}"
#             fields = f'bids={bids_str},asks={asks_str}'
#
#             ilp_line = f"{table_name},{tags} {fields} {timestamp_ns}\n"
#
#             # Queue for writing
